...
If the user specifies a different install path, use that instead of `~/tradingview-mcp-aro`.
...
```bash
cp ~/tradingview-mcp-aro/rules.example.json ~/tradingview-mcp-aro/rules.json
```
