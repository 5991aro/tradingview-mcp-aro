...
export function registerPineTools(server) {
  ...
  server.tool('pine_open', 'Open a saved Pine Script by name via TradingView\'s own Open dialog — this truly switches the editor\'s backing script, so a later pine_save goes to the right identity. Check the result\'s "method" field: "ui_open"/"already_open" = safe; "facade_fallback" = the UI flow failed and the source was only INJECTED into the currently open script (a save would overwrite it — see the "warning" field).', {
